...
class WindowMonitor:
    ...
    def __init__(self, db_callback, interval=10):
        ...
        self._running = False
        self._thread = None
        self._minute_buffer = {}  # { ("YYYY-MM-DD HH:MM", process_name): {title, process, duration} }
    ...
    def _loop(self):
        ...
        while self._running:
            try:
                ...
                buffer_key = (minute_key, process)

                # 检查是否有前一分钟的缓存需要 flush
                keys_to_flush = []
                for key in self._minute_buffer:
                    if key[0] != minute_key:
                        keys_to_flush.append(key)

                for key in keys_to_flush:
                    data = self._minute_buffer.pop(key)
                    try:
                        self.callback(
                            timestamp=key[0],
                ...
                # 同一分钟的采样累加时长
                if buffer_key in self._minute_buffer:
                    self._minute_buffer[buffer_key]["duration_seconds"] += self.interval
                else:
                    self._minute_buffer[buffer_key] = {
                        "window_title": title,
    ...
    def _flush_buffer(self):
        """将缓冲区的聚合记录写入数据库并清空"""
        for (minute_key, proc_name), data in list(self._minute_buffer.items()):
            try:
                self.callback(
